...
export default function CompanyPhysicBossCard({ dirigeant }) {
    ...
    return (
        <a
            href={`/?q=${encodeURIComponent(`${dirigeant.prenoms} ${dirigeant.nom}`)}`}
            className="hover:-translate-y-1 transition ease-in-out duration-150 hover:bg-gray-400 bg-gray-300 p-2 max-w-80 h-36 rounded-lg"
        >
